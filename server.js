          ...
          function () {
            spotifyApi.getMyCurrentPlaybackState().then(
                function(playback) {
                    ...
                    // client uses different description for different playing contexts
                    if(playlist.length == 0) {
                        io.sockets.emit('playing', { 'body': playback, 'empty': true, 'client_name': null });
                    } else if(playback.body.context == null) {
                        io.sockets.emit('playing', { 'body': playback, 'empty': false, 'client_name': null });
                    } else {
                        io.sockets.emit('playing', { 'body': playback, 'empty': false, 'client_name': playlist[queue_pos].client_name });
                    }
